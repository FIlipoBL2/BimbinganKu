document.addEventListener('DOMContentLoaded', () => {
    // --- Mock Data (Replace with actual API calls in Spring Boot) ---
    const MOCK_UNREAD_MESSAGES = [
        ...
        // Read messages would not be in the notification count
        { id: 103, sender: "Dr. Jones", message: "Session notes are ready.", read: true },
    ];
    const MOCK_SCHEDULE = [
        { id: 1, lecturer: 'Dr. X', time: '08:00', day: 'Tue', notes: 'Discuss chapter 3' },
        { id: 2, lecturer: 'Dr. Y', time: '09:00', day: 'Thu', notes: 'Thesis progress review' },
        // ... more sessions
    ];

    // --- Notification Logic (Requirement 2) ---
    ...
    if (unreadCount > 0) {
        ...
        notificationDropdown.innerHTML = ''; // Clear empty message
    ...
    const handleDaySelect = (selectedButton) => {
        ...
        // 1. Update mini-calendar highlight
        document.querySelectorAll('#calendar-days button').forEach(btn => {
            btn.classList.remove('selected');
        });
        selectedButton.classList.add('selected');

        // 2. Update big schedule column highlight
        scheduleTable.querySelectorAll('th, td').forEach(cell => {
            cell.classList.remove('highlighted');
        });

        const dayColumns = scheduleTable.querySelectorAll(`[data-day="${dayName}"]`);
        dayColumns.forEach(th => th.classList.add('highlighted'));
    ...
    // Attach click listeners to calendar buttons
    ...
    // Initialize highlight for the default selected day (e.g., today's column or default mockup day)
    const initialSelectedDay = document.querySelector('#calendar-days button.selected');
    if (initialSelectedDay) {
        handleDaySelect(initialSelectedDay);
    }
    
    // --- Session Management (New/Edit/Delete - Requirement 1) ---
    ...
    addSessionBtn.addEventListener('click', () => {
        ...
        // In a real app, you would populate the Lecturer dropdown here
    });
    ...
    scheduleTable.addEventListener('click', (event) => {
        ...
        if (sessionCell) {
            // In a real app, you'd fetch session details (e.g., from MOCK_SCHEDULE based on the cell's ID)
            // For a demo, we'll just open the modal with a simulated ID
            modal.classList.remove('hidden');
            ...
            // SIMULATE DATA POPULATION (Lecturer UI would also have Notes field enabled)
            // document.getElementById('session-date').value = '2025-11-27';
            // document.getElementById('session-time').value = '09:00';
        }
    });

    // Handle form submission (Simulate Save/Update)
    form.addEventListener('submit', (event) => {
        event.preventDefault();
        const sessionData = {
            lecturer: document.getElementById('session-lecturer').value,
            date: document.getElementById('session-date').value,
            time: document.getElementById('session-time').value,
        };
        
        console.log('Saving Session Data:', sessionData);
        alert('Session saved successfully! A notification should be sent to the involved parties.');
        modal.classList.add('hidden');
        
        // In a real app, you'd send this data to a Spring Boot API endpoint.
    });

    // Handle Delete button (Simulate Delete)
    document.getElementById('delete-session-btn').addEventListener('click', () => {
        if (confirm('Are you sure you want to delete this session?')) {
            console.log('Session deleted.');
            alert('Session deleted successfully! A notification should be sent to the involved parties.');
            modal.classList.add('hidden');
            // In a real app, you'd send a DELETE request to a Spring Boot API endpoint.
        }
    });
    ...
    // NOTE: You must also ensure that the session modal logic is still present 
    // (the custom logout modal logic you added previously should be kept intact).
    // NOTE: Session notes logic (Lecturer only edit, Student only view) 
    // must be handled on the Spring Boot side by checking the user role.
});